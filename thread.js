...
// class Thread {
//     constructor(fn, g = {}, scripts = [], base = '') {
//         const invoke = 'function ' + this.__invoke__.toString().replace('__invoke__', 'invoke');
//         const __reviver__ = 'function ' + this.__reviver__.toString();
//         const __replacer__ = 'function ' + this.__replacer__.toString();
//         const clone_source = `const Clone=(${CloneImplimentation.toString()})();Thread.transporter = Clone;`;
//         const blob = new Blob([`
// importScripts(${scripts.map(x=>'"'+Ajax.getAbsolutePath(base,x)+'"').join(',')});
// const invoke=${invoke};
// const __reviver__=${__reviver__};
// const __replacer__=${__replacer__};
// ${Ajax.toString()}
// ${Thread.toString()}
// Thread.transporter = JSON;
// ${clone_source}
// var g=Thread.transporter.parse('${Thread.transporter.stringify(g,true)}',__reviver__);
// const main=(${fn.toString()});
// const initonmessage=async function(msg){
//     var msg_obj=Thread.transporter.parse(msg.data,__reviver__);
//     for (const key in g) (0,eval)(key+'=g.'+key);
//     var result=await main.bind(msg_obj['this'],...msg_obj.args)();
//     switch(msg_obj.type){
//         case 'start':
...
//         return this;
//     }
//     _wakethread(args, type) {
//         return new Promise((res, rej) => {
//             this.worker.onmessage = __msg__ => {
//                 this.__worker_onmessage__(__msg__, res, rej);
//             };
//             this.worker.onerror = rej;
//             this.worker.postMessage(Thread.transporter.stringify({
//                 'this': this,
//                 args,
//                 type,
//             }));
//         });
//     }
//     __worker_onmessage__(__msg__, res, rej) {
//         switch (__msg__.data.type) {
//             case 'end':
//                 this.terminate();
//                 res(__msg__.data.data);
...
//                 break;
//             default:
//                 var __args__ = Thread.transporter.parse(__msg__.data.args, this.__reviver__);
//                 var decreation = '';
//                 for (const key in __args__) decreation += `var ${key} = __args__.${key};`;
//                 var __result__ = eval(`
//                     (function(__args__){
...
//                 type: 'invoke',
//                 data: fn.toString(),
//                 args: Thread.transporter.stringify(args),
//             });
//             self.onmessage = function (msg) {
//                 res(msg.data.data);
//             };
...
//         return new Promise(res => setTimeout(res, ms));
//     }
//     static any(threads, method = 'start') {
//         return Promise.race(threads.map(x => {
//             return x.args ? x[method](...x.args) : x[method]();
//         }));
//     }
//     static all(threads, method = 'start') {
//         return Promise.all(threads.map(x => {
//             return x.args ? x[method](...x.args) : x[method]();
//         }));
//     }
//     static create(num, fn, g, scripts) {
//         return Array.from({
//             length: num
//         }, () => new Thread(fn, g, scripts));
//     }
// }
// Thread.transporter = Clone // (typeof Clone === "undefined") ? JSON : Clone;
...
// var Ajax = Ajax || (function AjaxDeclaration() {
//     const global = (0, eval)('this');
//     global.AjaxDeclaration = AjaxDeclaration;
//     const Ajax = function () {};
